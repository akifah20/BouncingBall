...
def update(x,y,vx,vy,dt):
     #update state: new position = old position + velocity * dt
        x = x + vx * dt  #2 pixels horizontally per loop
        y = y + vy * dt #5 pixels vertically per loop
...
while is_running:
    ...
    dt = clock.tick(60) / 1000 #the / 1000 converts milliseconds → seconds. so now its pixels per second, rather than frame

    is_running = process_input(is_running)
    x,y,vx,vy = update(x,y,vx,vy,dt)
    render(x,y)
    display()
